...
namespace Rediscovery.Client.App.Service.Features.Configuration
{
    ...
    {
        private readonly Rediscovery.Shared.Configurations.Service.Models.RemoteResourceConfiguration _remoteResourceSettings;
        ...
        public DistributeConfig(ILoggerFactory loggerFactory, 
            IOptions<Rediscovery.Shared.Configurations.Service.Models.RemoteResourceConfiguration> remoteResourceSettings,
        ...
        private void HandleManager()
        {
            try
            {
                ...
                {
                    var config = new Rediscovery.Shared.Configurations.Manager.Models.ConnectionConfiguration
                    {
                    ...
                    OnUpdateRemoteConfiguration(Path.Combine(managerPath, Rediscovery.Shared.Configurations.Manager.ConfigFileNames.AppSettings), Rediscovery.Shared.Configurations.Manager.Models.ConnectionConfiguration.SectionName, config);
                }
            } catch (Exception ex)
            {
        ...
        private void HandleManagerGUI()
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(managerPath) && System.IO.Directory.Exists(managerPath))
                {
                    var config = new Rediscovery.Shared.Configurations.Manager.GUI.Models.ConnectionConfiguration
                    {
                        IP = _staticResources.HostIpAddress,
                        Port = _staticResources.HostPort,
                        DeviceIdentifier = _remoteResourceSettings.RediscoveryManagerDeviceIdentifier,
                    ...
                    OnUpdateRemoteConfiguration(Path.Combine(managerPath, Rediscovery.Shared.Configurations.Manager.GUI.ConfigFileNames.AppSettings), Rediscovery.Shared.Configurations.Manager.GUI.Models.ConnectionConfiguration.SectionName, config);
                }
            }
            catch (Exception ex)
            {
        ...
        private void HandleDiscoveryService()
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(discoveryPath) && System.IO.Directory.Exists(discoveryPath))
                {
                    ...
                    var serviceInfo = new Rediscovery.Shared.Configurations.Discovery.Models.ServiceInfoConfiguration
                    {
                        IP = _staticResources.HostIpAddress,
                        Port = _staticResources.HostPort,
                        MetaInfo = null,
                    ...
                    OnUpdateRemoteConfiguration(Path.Combine(discoveryPath, Rediscovery.Shared.Configurations.Discovery.ConfigFileNames.AppSettings), Rediscovery.Shared.Configurations.Discovery.Models.ServiceInfoConfiguration.SectionName, serviceInfo);

                    // Add firewall rule for discovery service
                    var fwDiscovery = OnReadRemoteConfiguration<Rediscovery.Shared.Configurations.Discovery.Models.DiscoveryConfiguration>(Path.Combine(discoveryPath, Rediscovery.Shared.Configurations.Discovery.ConfigFileNames.AppSettings), Rediscovery.Shared.Configurations.Discovery.Models.DiscoveryConfiguration.SectionName);
                    if (fwDiscovery != null)
                    {
        ...
        private void OnUpdateRemoteConfiguration<T>(string filePath, string key, T value)
        {
            try
            {
                Rediscovery.Shared.Configurations.RemoteConfiguration.UpdateRemoteConfiguration(filePath, key, value);
            } catch (Exception ex)
            {
                _logger.LogError(ex.ToString());
            }
        ...
        private T OnReadRemoteConfiguration<T>(string filePath, string key)
        {
            try
            {
                return Rediscovery.Shared.Configurations.RemoteConfiguration.ReadRemoteConfiguration<T>(filePath, key);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.ToString());
